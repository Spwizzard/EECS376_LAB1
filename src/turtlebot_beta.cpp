...
int main(int argc, char **argv) {
    ros::init(argc, argv, "turtlebot_beta"); 
    ...
    double speed = 0.305; // 1m/s speed command
    double yaw_rate = 0.5; //0.5 rad/sec yaw rate command
    double forward_time_3_sec = 3.5; // should move 3 meters or 1.5 rad in 3 seconds
    double turn_time_3_sec = 3.5; // should move 3 meters or 1.5 rad in 3 seconds
    ...
    while(timer<forward_time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
